...
namespace Rubicon.Data.DomainObjects.Linq.UnitTests.ParsingTest.SelectManyExpressionParserTest
{
  ...
  public class RecursiveSelectManyExpressionParserTest
  {
    ...
    public void ParsesFromExpressions ()
    {
      ...
      Assert.IsInstanceOfType (typeof (LambdaExpression), _parser.FromExpressions[2]);
      ...
      LambdaExpression fromExpression2 = (LambdaExpression) _parser.FromExpressions[2];
    ...
    public void ParsesFromIdentifiers ()
    {
      ...
      Assert.IsInstanceOfType (typeof (ParameterExpression), _parser.FromIdentifiers[2]);

      Assert.AreEqual ("s1", _parser.FromIdentifiers[0].Name);
      Assert.AreEqual ("s2", _parser.FromIdentifiers[1].Name);
      Assert.AreEqual ("s3", _parser.FromIdentifiers[2].Name);
    }
